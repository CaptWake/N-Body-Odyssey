  ...
  explicit Simulation(const json& sim_args) {
   if (sim_args["algorithm"] == "AP") {
      if (sim_args["mode"] == "SEQ") {
        ...
        if (sim_args.contains("ipath"))
          this->sim_ap_omp = OmpAP(sim_args["ipath"], sim_args["num_threads"], sim_args["schedule_type"], sim_args["chunk_size"]);
        else
   ...
   // Barnes Hut
     ...
     if (sim_args["mode"] == "SEQ") {
       ...
       if (sim_args.contains("ipath"))
         this->sim_bh_omp = OmpBH(sim_args["ipath"], sim_args["theta"], sim_args["num_threads"], sim_args["schedule_type"], sim_args["chunk_size"]);
       else
    ...
    while (t < time) {
      if (this->algorithm == "AP") {
        if (this->mode == "SEQ") {
          ...
          if (!fname.empty())
            this->sim_ap_omp.LogsToCSV(fname);
      ...
      // assuming Barnes Hut
        ...
        if (this->mode == "SEQ") {
          ...
          if (!fname.empty())
            this->sim_bh_omp.LogsToCSV(fname);
